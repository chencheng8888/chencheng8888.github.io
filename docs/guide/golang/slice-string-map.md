# String、Slice与Map的原理解析

## 一、string类型

### 1、字符的存储方式：

golang使用的是utf-8的编码方式，即不同编码所占用的字节数不同，具体来说，划分为[0,127],[128,2047],[2048,65535]，分别占用1字节，2字节和3字节，其编码模板具体来讲是下面这样的:
...
`string`包括`data`(来储存字符串的起始地址)和`len`(储存字符串的字节数，**注意不是长度**)，需要注意的是，字符串的内容允许读取，但不能像s[0]='0'这样修改，如果要修改字符串的内容，可以直接给字符串整体赋新值。

如果我需要修改字符串内容，第一个方法就是直接给字符串整体赋新值，第二个方法就是就是将字符串转成slice，这样slice会拷贝字符串的内容，这样就摆脱了字符串不能修改。

## 二、Slice类型

Slice有3个部分，分别为`data`（元素存哪里，**底层数组的起始地址**），len（存了多少个元素），cap（可以存多少个元素）。

如果像`var ints []int`这样声明一个切片，则这个切片的data为nil，len为0，cap为0 ，而如果像`var ints = make([]int,2,5)`声明切片，不仅会分配这三个结构，还会开辟一段内存作为它的底层数组。如果像`ints := new([]int)` 声明一个切片，就会data就会为nil，len为0，cap为0，此时其还没有底层数组，而通过`append`方式添加元素，它就会给slice开辟底层数组。

slice是可以共用底层数组元素，如下所示，s1和s2是共用arr的底层数组的，所以如果比如修改s1[0] = 2，那么arr[1]就会为2,但是修改s1[3]就是非法的，但是只要append给s1扩容就可以访问，并且还是使用arr的底层数组，还有种特殊情况，如果s2需要使用append扩容，它并不会在arr后再开辟一段内存，而是重新给s2分配一段内存，这是s2就不共用arr的底层数组了。
...
>  Slice已经存储的元素是可以安全读写的，但是超出范围就会出现panic。

### slice的扩容规则

| 情况                                                  | 扩容方式                                                     |
| ----------------------------------------------------- | ------------------------------------------------------------ |
| 如果oldCap * 2 < cap(所需最小容量)                    | newCap = cap                                                 |
| 如果oldCap * 2 >= cap (所需最小容量) 并且oldCap<256   | newCap = oldCap * 2                                          |
| 如果oldCap * 2 >= cap (所需最小容量) 并且oldCap>=1024 | newCap =  oldCap * 1.25  (一直做这个操作，直到得到的新容量大于预期的容量) |

### 扩容后需要多少内存？

扩容后分配的内存大小并不是简单地是`预估容量 * 元素类型大小`，在此之前我们需要了解的是，在许多编程语言中，申请分配内存并不是直接向操作系统交涉，而是和语言自身实现的内存管理模块，它会提前像操作系统申请一批内存，分成常用的规格管理起来，如8,16,32,48,64,80,96,112...  我们申请内存时，它会帮我们匹配足够大且最接近的规格。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xAy07QRq8ZOtpucRqp3pxGD2Z8rl4d0zIW1xCChDw7d28xvIN2cHQUA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xrFiaFIdLpXicxWjHF3kVswzNLzLzpgFb2zndSdCGZp211Tlno5nk7VNA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

（3）hash 的单向性：企图通过 hash 值反向映射回 key 是无迹可寻的.

![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xF5mTCAJ0cJkNtZIOSib0NDAd5zovB6CTfHzqW4dBwLqktNcr222QORg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

（4）hash 冲突：由于输入域（key）无穷大，输出域（hash 值）有限，因此必然存在不同 key 映射到相同 hash 值的情况，称之为 hash 冲突.

![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5x4ibdHHBfwicsgZ0KZCDKm1FJLpxocgMb67LvKECUUtB4lybhudKz92mA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5x7UAUibf0DBUJ65tTQjr7hMzDWuicyFjvFD8ZpMz1ukjGgiaFGY20jotwA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xE1OOMuX5AIEtl6cQeibcKNf9v2YibSeK6rnsncJKxqZ47aKicwwhBq0iaA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

（2）开放寻址法

开放寻址法中，在插入新条目时，会基于一定的探测策略持续寻找，直到找到一个可用于存放数据的空位为止.

![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xs80iagnj3wyMuib1ga8lOmrW2H8F59qJWBZuZvb51CE3az291L8C0Dzg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xN7Wzw5RZxrMkgSHUV1KySs3vNb4K68Z4SKiaf0YkxBzxm35xXx4BaSg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xKMnlcAK8ZnBea863au90G20QM9m2vXAvrLpa5iaibOQaibnPGyVkvrG2A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

### 3 数据结构

#### 3.1hmap

![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5x81hicw2xUichvfsXX8zNkpVmJ9Gib1RJeA6rKOYNHXqSBrraSiaeMewGbg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xVBxzza1lT8VeISlJW63XTQhdcHf8k2376I07lamIWKyTib0iccEaGkMQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xpfkKXezrzouDObRClehoILB8AYib0Kt1piaiahHib5pSN2EnFPn6RwbwtA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xjyPiaLXxITu6DCUvs6XQ6IRI1CdMoKU2cadficLhXro3bEHFSCGAxHjA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xfUNhlneXoL9DIWD6vC847icJ5UzI3WDZreNWd0aALSlVNJCSrp1qkbA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xQC9fQYDemJyrU5y1l8NDsiaUicLNv4MbHfHcADTczuQibtTDZDXetXZ8A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xgjib724BtVu6WUseExIOlSqJXNKxSahocdhwmibGPrA1aBBS5kAQQfDg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xCApLEbibEJUg5CTHYNgJeH8nw6X2ibtLkWm55ia8MBXmkmmoCaMsPwtwg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xtldnPazf9FkWhjY67JHmaicXjG0E64JbzA4vslTRupKV3FnUTicPKDAg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xBk4SR1Rc1oj3AHn95QftXDuuFqnJ4z4Ft00ViaqgTqy9hGs9WEIdV2Q/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xIc21mmCq7ibWoBY9TkJwa0NjcE7edEVUlgKYLrsOlgbBCxLZiagYIjyA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xGQ4uDFwF5OcI75CU0XqJorf8qgictnAicDqjjFnljJuvMnicJkx3RT1vg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
```
 ...
 ![图片](https://mmbiz.qpic.cn/mmbiz_png/3ic3aBqT2ibZuARxegbGpWRTtYV5T52c5xVwug95icdgSQMv2F6hYtjNLyQAvaL5O3xAKTiaD2NuicXia7A5ricbEuM7A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
